...
namespace UnitTest
{
    ...
    public:
        ...
        // T029: GetTextAttrForExtension tests

        TEST_METHOD(GetTextAttrForExtension_KnownCpp)
        {
            auto cfg = std::make_shared<CConfig>();
            cfg->SetEnvironmentProvider (&s_noOpEnv);
            cfg->Initialize (7);  // Default white-on-black



            WORD attr = cfg->GetTextAttrForExtension (L"C:\\src\\main.cpp");

            // .cpp should have a mapped color (not default)
            WORD defaultAttr = cfg->m_rgAttributes[CConfig::EAttribute::Default];
            Assert::AreNotEqual (defaultAttr, attr, L".cpp should have a non-default color");
        }



        TEST_METHOD(GetTextAttrForExtension_KnownPy)
        {
            auto cfg = std::make_shared<CConfig>();
            cfg->SetEnvironmentProvider (&s_noOpEnv);
            cfg->Initialize (7);
            ...
            WORD attr = cfg->GetTextAttrForExtension (L"script.py");

            WORD defaultAttr = cfg->m_rgAttributes[CConfig::EAttribute::Default];
            Assert::AreNotEqual (defaultAttr, attr, L".py should have a non-default color");
        }



        TEST_METHOD(GetTextAttrForExtension_UnknownExtension)
        {
            auto cfg = std::make_shared<CConfig>();
            cfg->SetEnvironmentProvider (&s_noOpEnv);
            cfg->Initialize (7);
            ...
            WORD attr = cfg->GetTextAttrForExtension (L"file.zzzzunknown");

            WORD defaultAttr = cfg->m_rgAttributes[CConfig::EAttribute::Default];
            Assert::AreEqual (defaultAttr, attr, L"Unknown extension should return default");
        }



        TEST_METHOD(GetTextAttrForExtension_NoExtension)
        {
            auto cfg = std::make_shared<CConfig>();
            cfg->SetEnvironmentProvider (&s_noOpEnv);
            cfg->Initialize (7);
            ...
            WORD attr = cfg->GetTextAttrForExtension (L"Makefile");

            WORD defaultAttr = cfg->m_rgAttributes[CConfig::EAttribute::Default];
            Assert::AreEqual (defaultAttr, attr, L"Extensionless path should return default");
        }
