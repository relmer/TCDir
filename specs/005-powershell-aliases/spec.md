...
**Independent Test**: Can be tested by setting up aliases in a profile, running `tcdir --get-aliases`, and verifying the output lists correct alias names, their mappings, and source locations.

**Acceptance Scenarios**:

1. **Given** aliases are stored in a profile file with tcdir marker comments, **When** the user runs `tcdir --get-aliases`, **Then** the output lists each alias name and its mapping, grouped by source file.
...
3. **Given** aliases exist in multiple profile files, **When** the user runs `tcdir --get-aliases`, **Then** each profile location is shown with its respective aliases listed underneath.
