...
- [x] T034c **Display State Preservation**: Add `SaveDirectoryState()`/`RestoreDirectoryState()` around child directory recursion in `TCDirCore/MultiThreadedLister.cpp` — preserves parent's `m_cchStringLengthOfMaxFileSize` and `m_fInSyncRoot` across recursion via `SDirectoryDisplayState` struct on `CResultsDisplayerTree` (see research.md R12). Note: the original design also preserved `m_fDisplayedFirstEntry`, `m_cVisibleEntries`, and `m_uliCurrentDirectoryTotalSize`, but these proved unnecessary since per-directory summaries are suppressed in tree mode (FR-019).
