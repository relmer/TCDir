    ...
    bool                     fPrintLeadingSeparator)
{
    ...
    //
    // Leading separator: produces 2 blank lines between profile dumps
    // (combined with the trailing blank from the previous profile).
    //

    if (fPrintLeadingSeparator)
    {
        console.Printf (CConfig::Information, L"\n");
    }

    console.Printf (CConfig::InformationHighlight, L"  %s  (%s)\n", loc.strVariableName.c_str(), loc.strResolvedPath.c_str());
    ...
    for (const auto & funcLine : block.rgFunctionLines)
    {
        console.Printf (CConfig::Information, L"    %s\n", funcLine.c_str());
    }

    //
    // Trailing blank line after the function list.
    //
...
HRESULT CAliasManager::GetAliases (CConsole & console)
{
    ...
    for (const auto & loc : rgLocations)
    {
        if (DisplayProfileAliases (console, fileMgr, loc, fFoundAny))
        {
            fFoundAny = true;
        }
    }
    ...
    if (!fFoundAny)
    {
        console.Printf (CConfig::Information, L"  No tcdir aliases found.\n");
        console.Printf (CConfig::Information, L"  Run 'tcdir --set-aliases' to configure aliases.\n");
        console.Printf (CConfig::Information, L"\n");
    }
